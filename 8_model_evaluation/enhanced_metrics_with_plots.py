# enhanced_metrics_with_plots.py
import pandas as pd
import numpy as np
import joblib
import os
...
class EnhancedMetricsVisualizer:
    """
    Enhanced version that creates comprehensive plots for all metrics.
    """
    
    def __init__(self):
        self.results = {}
        self.plots_dir = "enhanced_metrics_plots"
    ...
    def create_comprehensive_plots(self, metrics, model_name, target_column, class_names):
        ...
        self.plot_metrics_radar(metrics, base_name, target_column)
        ...
        for i in range(len(class_names)):
            for j in range(len(class_names)):
                ...
                ax.text(j + 0.5, i + 0.5, text, 
                       ha="center", va="center", 
                       color="red" if cm_percentage[i, j] > 50 else "black",
    ...
    def plot_per_class_metrics(self, metrics, class_names, base_name, target_column):
        ...
        if len(class_names) > 1:  # Only plot if we have multiple classes
            fig, ax = plt.subplots(figsize=(12, 8))
            ...
            bars1 = ax.bar(x - width, metrics['precision_per_class'], width, label='Precision', alpha=0.8)
            bars2 = ax.bar(x, metrics['recall_per_class'], width, label='Recall', alpha=0.8)
            bars3 = ax.bar(x + width, metrics['f1_per_class'], width, label='F1-Score', alpha=0.8)
    ...
    def plot_metrics_radar(self, metrics, base_name, target_column):
        """Create radar chart for metrics visualization."""
        try:
            from math import pi
            
            # Metrics for radar chart
            categories = ['Accuracy', 'Precision', 'Recall', 'F1-Score', 'Balance']
            values = [
                metrics['accuracy'],
                metrics['precision'],
                metrics['recall'],
                metrics['f1_score'],
                min(metrics['precision'], metrics['recall'])  # Balance metric
            ]
            ...
            plt.title(f'Performance Radar Chart: {base_name}\nTarget: {target_column}', 
                     fontsize=14, fontweight='bold', pad=20)
            plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.0))
            
            plt.tight_layout()
            plt.savefig(f'{self.plots_dir}/{base_name}_radar_chart.png', dpi=300, bbox_inches='tight')
            plt.close()
            
        except ImportError:
            print("   ⚠️  Radar chart skipped (math module required)")
    ...
    def create_performance_dashboard(self, metrics, model_name, target_column, class_names, base_name):
        ...
        bars = ax2.bar(metric_names, metric_values, color=['#2E86AB', '#A23B72', '#F18F01', '#C73E1D'])
        for bar, value in zip(bars, metric_values):
            height = bar.get_height()
        ...
        # 3. Per-Class Metrics (if multiple classes)
        if len(class_names) > 1:
            ax3 = fig.add_subplot(gs[0, 2])
            x = np.arange(len(class_names))
            width = 0.25
            
            ax3.bar(x - width, metrics['precision_per_class'], width, label='Precision', alpha=0.8)
            ax3.bar(x, metrics['recall_per_class'], width, label='Recall', alpha=0.8)
            ax3.bar(x + width, metrics['f1_per_class'], width, label='F1-Score', alpha=0.8)
    ...
    def print_results(self, metrics, model_name, target_column, class_names):
        ...
        print(f"\n📝 **Classification Report**:")
        print(classification_report(metrics['classification_report']['support'].keys(),
                                  [1] * len(class_names),  # dummy for display
                                  target_names=class_names,
                                  zero_division=0))
