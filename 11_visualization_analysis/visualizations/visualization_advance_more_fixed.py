...
class ProfessionalStockVisualizer:
    ...
    def create_fixed_volatility_analysis(self):
        """Fixed volatility analysis without 3D surface issues"""
        ...
        for file_name, df in self.dfs.items():
            if 'Close' in df.columns and len(df) > 100:
                ...
                for horizon in horizons:
                    ...
                    plt.plot(df['Date'], rolling_vol, label=f'{horizon}-day Volatility', alpha=0.8)
    ...
    def create_risk_metrics_dashboard(self):
        ...
        metrics_to_plot = ['Volatility (Annual)', 'Sharpe Ratio', 'Max Drawdown', 
                          ...
                          'Kurtosis', 'Auto-correlation (lag1)']
        
        for i, metric in enumerate(metrics_to_plot):
            if i < 9:  # Ensure we don't exceed subplot count
                row, col = i // 3, i % 3
    ...
    def run_professional_analyses(self):
        ...
        self.create_fixed_volatility_analysis()  # Fixed version
        ...
        print("Run this command to see all plot files:")
        print("ls -la *.png")
